        ...
        for (const item of chunks) {
          try {
            ...
            if (!inToolCall) {
              ...
              if (toolCallStart !== -1) {
                // 输出工具调用之前的文本内容
                const textBeforeTool = contentBuffer.substring(0, toolCallStart).trim()
                if (textBeforeTool) {
                  const textChunk = {
                ...
                inToolCall = true
                toolCallBuffer = contentBuffer.substring(toolCallStart + '<tool_call>'.length)
                contentBuffer = ''
              } else {
                ...
                if (contentBuffer.length > keepLength) {
                  const emitText = contentBuffer.substring(0, contentBuffer.length - keepLength)
                  contentBuffer = contentBuffer.substring(contentBuffer.length - keepLength)
              ...
              if (toolCallEnd !== -1) {
                // 提取完整的工具调用 JSON
                const toolCallJson = toolCallBuffer.substring(0, toolCallEnd).trim()
                ...
                try {
                      ...
                      'choices': [
                        {
                          ...
                          'delta': {
                            'tool_calls': [
                              {
                                'index': toolCallIndex,
                                'id': `call_${String(toolCallIndex).padStart(9, '0')}`,
                                'type': 'function',
                                'function': {
                                  'name': parsed.name,
                  ...
                  logger.warn(`Failed to parse tool call in stream: ${parseError.message}`, 'TOOLS_CHAT', '⚠️')
                }

                // 处理 </tool_call> 之后的内容
                const afterToolCall = toolCallBuffer.substring(toolCallEnd + '</tool_call>'.length)
                inToolCall = false
                toolCallBuffer = ''
    ...
    response.on('end', async () => {
      try {
        ...
        if (totalTokens.prompt_tokens === 0 && totalTokens.completion_tokens === 0) {
          ...
          logger.info(`工具调用流式使用tiktoken计算 - Prompt: ${totalTokens.prompt_tokens}, Completion: ${totalTokens.completion_tokens}, Total: ${totalTokens.total_tokens}`, 'TOOLS_CHAT')
        } else {
          logger.info(`工具调用流式使用上游真实Token - Prompt: ${totalTokens.prompt_tokens}, Completion: ${totalTokens.completion_tokens}, Total: ${totalTokens.total_tokens}`, 'TOOLS_CHAT')
        }
 ...
 * @param {object} requestBody - 原始请求体
 */
const handleToolsNonStreamResponse = async (res, response, model, requestBody = null) => {
  try {
    const decoder = new TextDecoder('utf-8')
    let buffer = ''
    let fullContent = ''
        ...
        try {
          ...
          for (const item of chunks) {
            try {
              ...
              logger.error('工具调用非流式数据处理错误', 'TOOLS_CHAT', '', error)
            }
          }
        } catch (error) {
    ...
    if (totalTokens.prompt_tokens === 0 && totalTokens.completion_tokens === 0) {
      ...
      logger.info(`工具调用非流式使用tiktoken计算 - Prompt: ${totalTokens.prompt_tokens}, Completion: ${totalTokens.completion_tokens}, Total: ${totalTokens.total_tokens}`, 'TOOLS_CHAT')
    } else {
      logger.info(`工具调用非流式使用上游真实Token - Prompt: ${totalTokens.prompt_tokens}, Completion: ${totalTokens.completion_tokens}, Total: ${totalTokens.total_tokens}`, 'TOOLS_CHAT')
    }
...
const handleToolsChatCompletion = async (req, res) => {
  ...
  try {
    ...
    // 将工具定义注入到消息中
    const injectedMessages = injectToolsIntoMessages(messages, tools, tool_choice)
    ...
    logger.info(`发送工具调用聊天请求，工具数量: ${tools.length}`, 'TOOLS_CHAT')
    ...
    if (stream) {
      ...
      await handleToolsNonStreamResponse(res, response_data.response, model, req.body)
    }
  } catch (error) {
    logger.error('工具调用聊天处理错误', 'TOOLS_CHAT', '', error)
    res.status(500)
